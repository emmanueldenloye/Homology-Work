...
data PType
  = HasATS {matrix' :: {-# UNPACK #-} !(Matrix Double)
...
determineATS n point m = ats . dropWhile isNothing . procProjs $ getKNNs
  where ats (Just m':_) = HasATS m' point
        ats _ = Boundary
        otherpts = on (\\) toRows
...
computeKNNs n = map (fromRows . map fst) . process
  where sorting = sortBy (comparing $ map snd) . map (sortBy $ comparing snd)
        collectDists = map ((,) <*> join (<.>))
        process = sorting . choose n . collectDists . toRows

centerMatrixRow
  :: forall s t.
     (Num t,Num (Vector t),Container Vector t)
